...
// ============================================================
router.patch("/:id", requireTenantAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { status, media_url } = req.body;
    ...
    if (error) {
      res.status(500).json({ error: "Database update error", details: error });
      return;
    }
    ...
    await supabase.from("complaint_events").insert({
      master_id: id,
      event_type: status === "resolved" ? "RESOLVED" : "STATUS_UPDATED",
      actor_id: "00000000-0000-0000-0000-000000000000", // System/Officer ID
      metadata: { new_status: status, media_url }
    });
