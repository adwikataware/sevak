  ...
  useEffect(() => {
    const fetchComplaints = async () => {
      try {
        ...
        // Map backend schema to frontend UI structure
        const mappedData = rawData.map(c => ({
          id: c.public_id || c.id,
          title: c.title,
          location: c.address || `${c.latitude.toFixed(3)}, ${c.longitude.toFixed(3)}`,
          ...
          daysPending: Math.floor((new Date() - new Date(c.created_at || new Date())) / (1000 * 60 * 60 * 24)),
          lat: c.latitude,
          lng: c.longitude,
          desc: c.description,
          status: c.status,
          ai: {
            severity: `${c.ai_verification_score || 'N/A'}% Confidence`,
            category: c.category,
          ...
          <div style={{ display: 'flex', gap: '1rem' }}>
            ...
            <div style={{ display: 'flex', background: 'var(--bg-tertiary)', borderRadius: '6px', padding: '0.25rem', border: '1px solid var(--border-color)' }}>
              <button 
                ...
                style={{ padding: '0.4rem 0.8rem', borderRadius: '4px', background: viewMode === 'list' ? 'var(--bg-secondary)' : 'transparent', color: viewMode === 'list' ? 'var(--primary)' : 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.85rem', fontWeight: 500, transition: 'all 0.2s', boxShadow: viewMode === 'list' ? '0 1px 3px rgba(0,0,0,0.05)' : 'none' }}
              >
                <List size={16} /> List
              </button>
              <button 
                onClick={() => setViewMode('map')}
                style={{ padding: '0.4rem 0.8rem', borderRadius: '4px', background: viewMode === 'map' ? 'var(--bg-secondary)' : 'transparent', color: viewMode === 'map' ? 'var(--primary)' : 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.85rem', fontWeight: 500, transition: 'all 0.2s', boxShadow: viewMode === 'map' ? '0 1px 3px rgba(0,0,0,0.05)' : 'none' }}
              >
                <MapIcon size={16} /> Map
              </button>
              <button 
                onClick={() => setViewMode('analytics')}
                style={{ padding: '0.4rem 0.8rem', borderRadius: '4px', background: viewMode === 'analytics' ? 'var(--bg-secondary)' : 'transparent', color: viewMode === 'analytics' ? 'var(--primary)' : 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.85rem', fontWeight: 500, transition: 'all 0.2s', boxShadow: viewMode === 'analytics' ? '0 1px 3px rgba(0,0,0,0.05)' : 'none' }}
              >
        ...
        {selectedComplaint ? (
          <>
            ...
            <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <button className="btn-primary" style={{ justifyContent: 'center' }}>Confirm & Dispatch Officer</button>
              <button className="btn-secondary" style={{ justifyContent: 'center', color: 'var(--danger)', borderColor: 'var(--danger-light)' }}>Override Assignment</button>
            </div>
          </>
        ) : (
