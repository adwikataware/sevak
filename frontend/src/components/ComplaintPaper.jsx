...
import { Camera, MapPin, Mic, Send, Check, Image as ImageIcon, MessageCircle } from 'lucide-react';
import './ComplaintPaper.css';
    ...
    try {
      const response = await fetch('http://localhost:3000/api/complaints', {
        ...
        body: JSON.stringify({
          title,
          description,
          latitude: 18.5204 + (Math.random() - 0.5) * 0.01, // Simulate GPS for now
          longitude: 73.8567 + (Math.random() - 0.5) * 0.01,
          category: "General", // Will be refined by backend RAG
          tenantId: 'pune-slug'
        })
      });
      ...
      if (data.complaint) {
        setTicketId(data.complaint.public_id);
      } else {
        setTicketId('CIV-ERR-500');
      }
    } catch (error) {
          ...
          >
            <div className="paper-document">
              ...
              <form onSubmit={handleSubmit} className="paper-form">
                ...
                <textarea 
                  className="paper-field paper-textarea" 
                ...
                <AnimatePresence>
                  {imageAttached && (
                    ...
                    >
                      <div style={{ width: '60px', height: '60px', borderRadius: '4px', background: 'url(https://images.unsplash.com/photo-1515162816999-a0c47dc192f7?w=100&q=80) center/cover', border: '1px solid var(--border-color)' }} />
                      <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <span style={{ fontSize: '0.85rem', color: 'var(--primary)', fontWeight: 500, fontFamily: 'Inter' }}>IMG_4921.jpg</span>
                        <span style={{ fontSize: '0.75rem', color: 'var(--success)', fontFamily: 'Inter' }}>GPS Metadata Verified</span>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
      ...
      <AnimatePresence>
        {step === 'submitted' && (
          ...
          >
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem', fontFamily: 'Inter' }}>Your issue is securely in the system.</p>
